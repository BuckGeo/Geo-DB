...
df = pd.read_excel(r'C:\Users\H277910\New Geo Tracker.xlsx')
...
df2 = pd.read_excel(r'C:\Users\H277910\PycharmProjects\pythonProject1\SAGD Dashboard V2.xlsx')
...
data2 = df2.loc[df2['R/NR/Waiting'].ne('Waiting') & df2['R/NR/Waiting'].notna()]

data2=data2.groupby('CUSTOMER').filter(lambda x: (x['R/NR/Waiting'] == 'NR').sum() >= 1)

customers2 = data2['CUSTOMER'].unique()

# convert the customer names to the required format for dcc.Dropdown options
options2 = [{'label': 'All Customers', 'value': 'all'}] + \
          [{'label': customer, 'value': customer} for customer in customers2]
...
tab2_layout = html.Div(
    ...
    children=[
        ...
        dcc.Graph(id='failure-points-pie-chart2'),
        dcc.Graph(id='reason-for-pull-pie-chart2'),
        dcc.Graph(id='normal-dist-plot2'),
        dcc.Graph(id='survive-plot2'),
        dcc.Graph(id='reliability-plot2'),
        html.Footer(children=[html.P("Summit ESP - Global Technical Service; Created by Buck Pettit - 2023",
                                     style={"font-size": "small"})])
    ])
...
@app.callback(
    [dash.dependencies.Output('data-table', 'figure'),
        dash.dependencies.Output('failure-points-pie-chart2', 'figure'),
     dash.dependencies.Output('reason-for-pull-pie-chart2', 'figure')],
    dash.dependencies.Output('normal-dist-plot2', 'figure'),
    dash.dependencies.Output('survive-plot2', 'figure'),
    dash.dependencies.Output('reliability-plot2', 'figure'),
    [dash.dependencies.Input('seal-dropdown2', 'value'),
     dash.dependencies.Input('customer-dropdown2', 'value')]
)
def update_pie_charts(selected_seal_version, selected_customer):
    ...
    table_fig2.update_layout(height=450)

    
    if sum(failure_points)<0.9:
        failure_points_fig2 =px.pie(title='')
        failure_points_fig2.update_layout(title='No Failure Data', title_x=0.5,  # center the title horizontally
    title_font=dict(size=34))

        reason_for_pull_fig2 = px.pie()
        normal_dist_fig2 = px.pie()
        survivability_fig2 = px.pie()
        reliability_fig2 = px.pie()
        ...
        normal_dist_fig2 = px.scatter(
            x=sorted_runtime,
            y=pdf_values,
            title=f'Normal Distribution of Runtime (Total: {sum(failure_points)})'
        )
        normal_dist_fig2.update_layout(
            xaxis_title="Days",
            yaxis_title="Normal Distribution",
            shapes=[
                dict(
        ...
        for i in range(len(sorted_runtime)):
            results = 1 - (failure_count[i] / len(failure_count))
            prob_array.append(results)
        survivability_fig2 = px.line(x=sorted_runtime, y=prob_array,
                                    title=f'Survivability Curve for {selected_seal_version} (Total: {sum(failure_points)})')
        survivability_fig2.update_layout(
            xaxis_title="Days",
            yaxis_title="Survival Probability",
        )
        Total_failures = np.max(failure_count)
        ...
        for i in range(len(sorted_runtime)):
            reliability.append(math.exp((-1 / mttf) * sorted_runtime[i]))
        reliability_fig2 = px.line(x=sorted_runtime, y=reliability,
                                  title=f'Reliability Curve for {selected_seal_version} (Total: {sum(failure_points)})')
        reliability_fig2.update_layout(
            xaxis_title="Days",
            yaxis_title="Overall Reliability", )

    return table_fig2, failure_points_fig2, reason_for_pull_fig2, normal_dist_fig2, survivability_fig2, reliability_fig2

# Define the app layout with the tabs
app.layout = html.Div([
